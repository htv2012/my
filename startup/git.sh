...
# With too many aliases, we need a way to show aliases for a particular
# sub command such as "git push"
#     Example: glias push
glias() {
    alias | grep "git $1"
}

# Echoes an asterisk if the repository is dirty, untracked files are ignored
git_dirty_marker() {
    if [ -n "$(command git status --porcelain --untracked-files=no 2> /dev/null)" ]
    then
        echo "*"
    fi
}
...
gcm() {
    ...
    then
        branch=master
    elif git branch | grep "main" > /dev/null 2>&1
    then
        branch=main
    else
